...
from inference_postprocess import convert_to_csv
...
def get_args_parser():
    ...
    # image path
    parser.add_argument('--img_dir', type=str, default='demo/vg/',
...
def main(args):
    ...
    def rescale_bboxes(out_bbox, size):
        ...
        b = b * torch.tensor([img_w, img_h, img_w, img_h], dtype=torch.float32, device=args.device)
        return b
    ...
    img_paths = [img_dir_path + x for x in img_paths]
    for img_path in img_paths:
        ...
        with torch.no_grad():
            ...
            for idx, ax_i, (sxmin, symin, sxmax, symax), (oxmin, oymin, oxmax, oymax) in \
                ...
                rel_tuple = (CLASSES[probas_sub[idx].argmax()], REL_CLASSES[probas[idx].argmax()], CLASSES[probas_obj[idx].argmax()])


                rel_tuples.append(rel_tuple)

            print(img_path.split('/')[-1][:-4], rel_tuples)
        
            sgg_output.append([img_path.split('/')[-1][:-4], rel_tuples])

            fig.tight_layout()
            # plt.savefig('./reltr_output.png')
            # plt.show()

    print('FINAL SGG OUTPUT:')
    print(sgg_output)
    convert_to_csv(sgg_output, 'reltr-topk20-output.csv')
