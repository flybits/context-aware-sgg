...
def main(args):
    ...
    with torch.no_grad():
        ...
        for idx, ax_i, (sxmin, symin, sxmax, symax), (oxmin, oymin, oxmax, oymax) in \
            ...
            ax = ax_i[0]
            ax.imshow(dec_attn_weights_sub[0, idx].cpu().view(h, w).numpy())
            ax.axis('off')
            ax.set_title(f'query id: {idx.item()}')
            ax = ax_i[1]
            ax.imshow(dec_attn_weights_obj[0, idx].cpu().view(h, w).numpy())
            ax.axis('off')
            ax = ax_i[2]
            ax.imshow(im)
            ...
            rel_tuples.append((CLASSES[probas_sub[idx].argmax()], REL_CLASSES[probas[idx].argmax()], CLASSES[probas_obj[idx].argmax()]))
