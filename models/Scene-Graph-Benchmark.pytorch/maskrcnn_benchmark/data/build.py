...
from maskrcnn_benchmark.utils.custom_dataloader import CustomSubset
...
def get_dataset_statistics(cfg):
    ...
    try:
        ...
        logger.info('Saving new data statistics to: ' + str(save_file))
...
def make_data_loader(cfg, mode='train', is_distributed=False, start_iter=0, dataset_to_test=None, limit=None):
    ...
    if limit is not None:
        datasets = [CustomSubset(dataset, range(min(limit, len(dataset)))) for dataset in datasets]
