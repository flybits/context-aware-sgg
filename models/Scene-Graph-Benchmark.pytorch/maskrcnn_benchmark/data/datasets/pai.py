...
class paiDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        print(img_dir, roidb_file, image_file, "HIIIIIIIII")
        ...
        print(f"Dataset initialized with {len(self.filenames)} images.")
...
def get_VG_statistics(img_dir, roidb_file, dict_file, image_file, must_overlap=True):
    train_data = paiDataset(split='train', img_dir=img_dir, roidb_file=roidb_file, 
...
def load_image_filenames(img_dir, image_file):
    ...
    for i, img in enumerate(im_data):
        # Use the 'url' field to get the filename
        filename = img['url']
    ...
    print(f"Number of images found: {len(fns)}")
    return fns, img_info
